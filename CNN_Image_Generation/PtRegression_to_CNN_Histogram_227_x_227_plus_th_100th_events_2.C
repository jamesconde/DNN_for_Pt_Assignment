...
#include <fstream>
#include <time.h>
void PtRegression_to_CNN_Histogram_227_x_227_plus_th_100th_events_2()
{
	//Source data
	...
	int numEvents = 200000;//User set number of events
	cout<<">>>>>>>>>>>>>>>>>>>>>"<<endl;
        cout<<numEvents<<" events to process..."<<endl;
	
	//pT values file
	//TString outFolder = "/storage1/users/eb8/time_test/";
	TString outFolder = "/storage1/users/eb8/CNN_Images_227_227_plus_th_100th_x_10_events_Mar_1_2018/";
	TString outFile = outFolder + "pT_Values_1.csv";
	...
	for(Long64_t iEntry = 0; iEntry <numEvents; iEntry++){
		...
		//load the i-th entry
        	myTree->GetEntry(iEntry+200000);
		
		//Set up canvas
   		TString canv;
   		canv.Form("c%d",int(iEntry+200000));
		...
		//Set up histogram
		TString h_title;
		h_title.Form("hist%d",int(iEntry+200000));
		...
		//Save images
                ...
                TString img_name;
                img_name.Form("event%06d.png",int(iEntry+200000));
