...
def create_model(x_train, y_train, x_test, y_test):
    ...
    model.compile(loss='catagorical_crossentropy', metrics=['accuracy'],
...
if __name__ == '__main__':
    ...
    model_predictions = best_model.model.predict(X_test)
    ...
    np.savetxt(outfile_predict, model_predictions)
    np.savetxt(outfile_truth, Y_test)
    #plot_ROC(Y_test,model_predictions,1,show_toggle = True, save_toggle=True)
    # serialize model to JSON
    #model_json = model.to_json()
    #with open("model.json", "w") as json_file:
    #    json_file.write(model_json)
    # serialize weights to HDFt
    best_model.save_weights("/scratch/rice/DUMMYmodel.h5")
    print("Saved model to disk")
    best_model.save('/scratch/rice/DUMMY.h5')
